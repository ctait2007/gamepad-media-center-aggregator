...
// NuvioTV is laid out at density 2 on a 1080p screen, so its dp double into our
// 1:1 design units. Row: 14dp horizontal / md (12dp) vertical padding, a 22dp
// leading glyph, lg (16dp) to the text, bodyLarge title over a bodySmall
// subtitle spaced xxs (4dp), then labelLarge value / 20dp chevron.
constexpr float kPadX = 28;
constexpr float kPadY = 24;
constexpr float kMinHeight = 108;
...
// The toggle: 46x24dp track, xxs (4dp) padding, a 20dp white knob.
...
/// The grey NuvioTV uses for a subtitle, a value and a trailing chevron
/// (TextSecondary). Borealis' own list-value colour is the accent, which the
/// reference reserves for the focus ring and for a selected option.
...
Parts skin(brls::Box* row, brls::Label* title, brls::Label* value) {
    ...
    row->setMinHeight(kMinHeight);
    row->setPaddingTop(kPadY);
    row->setPaddingBottom(kPadY);
    row->setPaddingLeft(kPadX);
    row->setPaddingRight(kPadX);
    row->setAlignItems(brls::AlignItems::CENTER);
    // Focus is a ring, not a fill: containerColor and focusedContainerColor are
    // the same colour in the reference.
...
void setIcon(brls::View* row, const Parts& parts, const std::string& name) {
    ...
    // NuvioTV tints the glyph with the accent while the row is focused and with
    // TextSecondary the rest of the time.
    auto repaint = [parts, row, d]() {
        const bool focused = row->isFocused();
        parts.icon->setImageFromSVGString(svg24(d, focused ? themeColor("color/app") : secondaryText()));
    };
    repaint();
...
void keepPillRing(brls::View* row) { row->setHighlightCornerRadius(row->getHeight() / 2); }

SVGImage* makeChevron() {
    return makeGlyph("M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z", secondaryText(), kTrailingSize);
}

SVGImage* makeExpandMore() {
    return makeGlyph("M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6z", secondaryText(), kTrailingSize);
}
...
SettingsSection::SettingsSection() : brls::Box(brls::Axis::COLUMN) {
    ...
    settings_row::Parts headerParts = settings_row::skin(this->header, this->headerTitle, this->headerValue);
    this->headerSubtitle = headerParts.subtitle;
    this->headerIcon = settings_row::makeChevron();
...
void SettingsSection::updateHeader() {
    ...
    this->headerIcon = this->expanded ? settings_row::makeExpandMore() : settings_row::makeChevron();
    this->header->addView(this->headerIcon);
}
