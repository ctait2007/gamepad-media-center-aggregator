...
RecyclingGridItem* VideoDataSource::cellForRow(RecyclingView* recycler, size_t index) {
    ...
    // modernLandscapePostersEnabled: the card is 16:9 and shows the BACKDROP
    // with the item's logo over it, rather than the 2:3 poster. Read once —
    // the geometry behind it is fixed at startup (AppConfig::initThemes) and
    // the two must not disagree mid-row.
    static const bool kLandscape = AppConfig::instance().getItem(AppConfig::LAYOUT_LANDSCAPE_POSTERS, false);
    cell->applyLandscape(kLandscape, item);
    ...
    if (item.type == plex::mediaTypeEpisode) {
        ...
        // capture (portrait cards, user feedback). A LANDSCAPE card wants the
        // 16:9 art instead, which is what the episode's own thumb is.
        if (kLandscape && !cell->landscapeArt(item).empty()) {
            Image::load(cell->picture, cell->landscapeArt(item), 500);
        ...
        // the composite mosaic generated by the server. A landscape card puts
        // the backdrop first and the poster behind it, as the reference does.
        if (kLandscape && !cell->landscapeArt(item).empty()) {
            Image::load(cell->picture, cell->landscapeArt(item), 500);
        } else if (!item.thumb.empty()) {
            Image::load(cell->picture, item.thumb, 325);
